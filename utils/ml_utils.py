...
class BinaryClassifiersAnalysis:
    ...
    def plot_score_distribution(self, model_name, shade=False):
        ...
        legend = axs[1].legend(loc='best', shadow=True, fontsize='x-small')
...
-----------------------------------
"""
def cf_matrix_roc_auc(model, y_true, y_pred, y_pred_proba, feature_importances, X_data,roc_auc=0.34):
  ...
  an sklearn Confusion Matrix using a Seaborn heatmap visualization + ROC Curve.'''
    ...
    if (feature_importances != None) and (X_data != None):
        plt.subplot(212)
...
def classify_train_models(model, X_train, X_test, y_train, y_test):
    """
    Renvoie  es différentes score des models.
    on a les metrics suivantes :
    - AUC : roc_auc_score
...
def classify_custom_score(y_true, y_pred, beta=3) :
...
def classify_cost_false(model, y_true, X_true):
